<template>
  ...
  >
    <el-table-column prop="materialId" fixed label="FB图片编号" width="160"></el-table-column>
    <el-table-column prop="materialUrl" label="图片" width="120">
      <template slot-scope="scope">
        <div class="imageline">
          <span class="headpic">
            <img :src="scope.row.materialUrl" alt @click="showBig(scope.row.materialUrl)">
          </span>
          <i
            class="el-icon-download download"
            @click="downloadFile(scope.row.materialUrl)"
...
export default {
      ...
      columns.forEach((column, index) => {
        switch (index) {
          ...
          case 1:
            ...
            break;
          default:
            sums[index] = this.imagemattersum[keyarr[index - 2]];
            break;
        }
      });
  ...
  computed: {
    ...mapState(["imagematterlist", "imagemattersum"])
  }
};
</script>
